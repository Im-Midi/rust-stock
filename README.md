...
### 2026-06-13（K 线「AI 预测」推演下一交易日）
- **「AI 解读」升级为「AI 预测」**：把本股全部已知真实数据（行情/筹码/通达信指标/财务/估值/概念板块/近 5 日主力净流入/两融/龙虎榜 + 上一收盘价）喂给 AI，推演**下一交易日**的 K 线四值（开/高/低/收）。
- **新增 K 线蜡烛（紫色叠加）**：在最新真实蜡烛右侧追加一根**紫色虚线空心**预测蜡烛，配虚线连接线（上一收盘→预测收盘）与「AI预测」标签；纯叠加，不改真实蜡烛绘制；切股/切周期/重载即清除。
- **合规护栏（硬性）**：专用保守推演 prompt（"推演/假设"，绝不"必涨必跌/荐股"）；后端严格遵守 A 股涨跌停（主板 ±10% / 创业板·科创板 ±20% / 北交所 ±30%，按代码前缀判定）并校验蜡烛约束（高≥max(开,收)、低≤min(开,收)）；预测附**置信度（高/中/低）+ 一句依据**；图下方常驻**醒目免责声明**："⚠️ AI 预测为模型基于历史数据的推演，极可能出错，仅供参考，绝不构成投资建议。" 单根蜡烛的 LLM 预测准确度本就很低，仅作推演演示。
- 桌面 + 安卓双端生效。

## 2026-06-12（v1.5 发布 🎉 自 v1.0 以来的大版本）

> 一句话：从"能看行情"进化到"**数据驱动的 A 股 AI 投研浮窗**"——12 个真实数据维度喂进 AI，手机端取数全面加固。

### 今日 AI 推荐（引擎质变）
- **两阶段·逐股深度调研**：先初选 6–8 支，再对每支单独跑产业链八层深调（与「研」同源），结论并入推荐理由。
- **真实数据注入推荐引擎**：每支入选股深调前并发拉取**真实财报（营收/净利+同比/EPS/ROE/毛利率/负债率）、估值（总市值/PE/PB）、概念板块归属、近 5 日主力净流入、近 30 日龙虎榜**，作为"已查到的事实"注入——五大门派不再靠记忆瞎猜。方法论模板一字未动。
- **剔除连续 6 日逐日下行（6 连阴）**的标的。

### 新数据维度（K 线个股详情 / 行情页，均注入 AI 解读）
- **筹码分布图**（换手率衰减 + 成本中枢，获利比例/平均成本）、**通达信口径技术指标**（MACD/KDJ/RSI/BOLL 金叉死叉）。
- **概念/板块归属、历史资金流、核心财务、融资融券余额、个股龙虎榜、分红、股本市值、🔥 人气榜（点击进 K 线）、北向资金成交额**。

### 体验 / 可靠性
- **手机端取数大修**：K 线**三源兜底（东财→腾讯→新浪）**、资金流/换手率腾讯兜底、共享缓存 + last-good 不白屏、**后台并发预热 + 切页秒开**、安卓回前台秒级保温；真机失败**绝不再用假数据冒充**。
- **涨跌报警 + 真实节假日交易日历**、非交易时段红字提示、**财联社电报快讯 + 本地情绪词典**（零成本利好/利空标）、调研收藏一键复制、**液态玻璃外观**（设置可选）。

> 桌面（Windows/macOS）+ 安卓同源同体验。所有 AI 输出仅为研究参考，不构成投资建议。

## 2026-06-12（推荐引擎二阶段数据注入：单股深度调研接入真实基本面/资金数据）

### 新增
- **「今日 AI 推荐」二阶段单股深调接入数据注入层**：`deep_research_stock` 在组装提示词前，对该股并发（`tokio::join!`，单股内 5 路并行）拉取真实数据——核心财务指标（最新报告期：营收/净利+同比 · EPS · ROE · 毛利率 · 负债率）、估值快照（总市值 · PE(动) · PB）、所属概念/板块（行业排最前）、近 5 日主力净流入（逐日+合计）、近 30 日龙虎榜明细（上榜日 · 原因 · 净买额，仅近期上过榜才注入）——拼成紧凑的「【实时真实数据】」块注入 user 消息。价值派/成长派的深调从此基于真实财报与资金数据，而非模型的陈旧记忆。
- 全部复用 `extra.rs` 既有 fetcher（K线页「AI 解读」同源，均走 `em_get_text` 免 gzip + 3 次退避重试）；任一接口失败只缺对应行、绝不拖垮该股深调，全部失败时提示词与旧版完全一致（优雅降级）。防幻觉指令同步收紧：「涉及具体数字以上文【实时真实数据】与给定行情为准；未提供的数据不得编造，确需提及时注明未获取到」。
- `extra.rs` 新增免 chrono 的 `days_from_civil` / `days_since` 日期工具（Hinnant civil 算法，带单测），供后端按"近 30 日"口径过滤龙虎榜（与 K线页前端同口径）。

### 说明
- RESEARCH_SYSTEM 方法论 / 分节模板 / 五大门派提示词均未改动——纯数据注入层。一阶段 60 只候选池逻辑不变；仅二阶段 ≤8 只深调每股增加 5 个并发取数（每次推荐约 +40 个东财请求，推荐本身低频且分钟级耗时，可接受）。两融、分红两项为控制请求量暂未注入（fetcher 已就绪，需要时一行接入）。


## 2026-06-12（第六批：三个 A 股数据维度——核心财务指标 + 融资融券 + 个股龙虎榜）

> 接口设计继续参考 1nchaos/adata（Apache-2.0，仅作接口字典，未复制代码）；全部接口均经真实请求复核返回形状后才接入。这三项把 AI 解读从"纯盘面"升级为"盘面 + 真实基本面 + 杠杆/游资资金"。

### 新增
- **核心财务指标（K线页个股详情 + AI 上下文）**：东财 F10 `RPT_F10_FINANCE_MAINFINADATA`（主要财务指标，按报告期倒序取最新一期）。快照格下方一行小字：「财报 2026一季报 · 营收 547.0亿(+6.3%) · 净利 272.4亿(+1.5%) · EPS 21.76 · ROE 10.6% · 毛利 89.8% · 负债 12.1%」。字段：`TOTALOPERATEREVE/TZ` 营收+同比、`PARENTNETPROFIT/TZ` 归母净利+同比、`EPSJB` 基本每股收益、`ROEJQ` 加权ROE、`XSMLL` 毛利率、`ZCFZL` 资产负债率。未披露字段（如银行无毛利率）后端给 null、前端整段省略——绝不把"未披露"伪装成 0。按日缓存（`fin_<code>`），失败沿用旧缓存，没有就整行隐藏。AI 解读上下文同步注入一行，价值派/成长派从此有真实财报可依。
- **融资融券个股余额（K线页个股详情 + AI 上下文）**：东财 datacenter `RPTA_WEB_RZRQ_GGMX`（个股两融明细，取最近 2 个交易日）。一行小字：「两融(06-11) 融资余额 191.9亿 -0.69亿 · 融券 1.7亿 · 合计 193.6亿」，融资余额较上日增减红绿着色。字段：`RZYE` 融资余额 / `RQYE` 融券余额 / `RZRQYE` 两融合计 / `RZJME` 当日融资净买额（单位元）。注意：adata 字典里的 `RPT_RZRQ_LSHDETAIL` 在 web datacenter 已不存在（实测回"报表配置不存在"），现行报表为 `RPTA_WEB_RZRQ_GGMX`。非两融标的（datacenter 回 `code:9201 返回数据为空`）后端如实返回空数组 → 前端隐藏该行且按日缓存（`margin_<code>`），与取数失败严格区分。
- **个股龙虎榜（K线页个股详情 + AI 上下文，仅近期上榜才显示）**：东财 `RPT_DAILYBILLBOARD_DETAILSNEW` 按 `SECURITY_CODE` 过滤（此前候选池只用它拉全市场当日上榜 codes，这次取个股明细：上榜日 `TRADE_DATE`、上榜原因 `EXPLANATION`、席位净买额 `BILLBOARD_NET_AMT`、买入/卖出额）。**近 30 日上过榜**才显示一行：「龙虎榜 06-04 净买 +10.67亿 · 连续三个交易日内收盘价格涨幅偏离值… · 近30日2次」（净买额红绿着色、原因去模板前缀+截断）；从未上榜（9201）/久未上榜 → 整行不出现，绝不硬凑。按日缓存（`lhbd_<code>`）。AI 上下文注入完整上榜原因 + 净买/买入/卖出 + 近30日次数，游资打板视角从此有真实席位数据。
- 三个维度的 Rust 解析器（`parse_financials` / `parse_margin_days` / `parse_lhb_detail`）均带 2026-06-12 实测真实形状单元测试，并覆盖"非两融标的/从未上榜（9201）"与"响应无效"的诚实区分、财务字段 null 不造 0。

### 说明
- AI prompt 模板与五大门派 system 提示词均未改动——只是把更多真实数据注入 `context`。
- 三项均为按日数据（财报/两融/龙虎榜每日至多更新一次），按日 SQLite 缓存 + 过期缓存离线兜底，不增加盘中请求压力。
